...
def test_parse_and_merge_rootdir():
    result = _parse_and_merge(["--rootdir", "/tmp"], {"rootdir": None})
    assert result.get("rootdir") == "/tmp"


def test_parse_and_merge_collect_only():
    result = _parse_and_merge(["--collect-only"], {"collect_only": False})
    assert result.get("collect_only") is True


def test_parse_and_merge_strict_markers():
    result = _parse_and_merge(["--strict-markers"], {"strict_markers": False})
    assert result.get("strict_markers") is True


def test_parse_and_merge_showlocals():
    result = _parse_and_merge(["--showlocals"], {"showlocals": False})
    assert result.get("showlocals") is True


def test_parse_and_merge_setup_show():
    result = _parse_and_merge(["--setup-show"], {"setup_show": False})
    assert result.get("setup_show") is True


def test_parse_and_merge_lf():
    result = _parse_and_merge(["--lf"], {"lf": False})
    assert result.get("lf") is True


def test_parse_and_merge_ff():
    result = _parse_and_merge(["--ff"], {"ff": False})
    assert result.get("ff") is True


def test_parse_and_merge_cache_clear():
    result = _parse_and_merge(["--cache-clear"], {"cache_clear": False})
    assert result.get("cache_clear") is True


def test_parse_and_merge_pdb():
    result = _parse_and_merge(["--pdb"], {"pdb": False})
    assert result.get("pdb") is True


def test_parse_and_merge_trace():
    result = _parse_and_merge(["--trace"], {"trace": False})
    assert result.get("trace") is True


def test_parse_and_merge_ignore():
    result = _parse_and_merge(["--ignore", "tests/old"], {"ignore": []})
    assert result.get("ignore") == ["tests/old"]


def test_parse_and_merge_maxfail():
    result = _parse_and_merge(["--maxfail", "3"], {"maxfail": None})
    assert result.get("maxfail") == 3


def test_parse_and_merge_nocapture():
    result = _parse_and_merge(["-s"], {"nocapture": False})
    assert result.get("nocapture") is True


def test_parse_and_merge_version():
    result = _parse_and_merge(["--version"], {"version": False})
